  ...
  diseases.forEach(disease => {
    if (disease.category === 'negligenciada') {
      ...
      card.addEventListener('click', () => {
        window.location.href = `detalhe-doenca.html?doenca=${encodeURIComponent(disease.name)}&origem=negligenciadas`;
      });
...
function populateChronicDiseases() {
  ...
  diseases.forEach(disease => {
    if (disease.category === 'cronica') {
      ...
      `;
      card.addEventListener('click', () => {
        window.location.href = `detalhe-doenca.html?doenca=${encodeURIComponent(disease.name)}&origem=cronicas`;
      });
      section.appendChild(card);
    }
  });
  ...
  medicineMap.forEach(medicine => {
    ...
    card.innerHTML = `
      ...
      <div class="med-buttons">
        <button onclick="window.location.href='detalhe-medicamento.html?medicamento=${encodeURIComponent(medicine.name)}&origem=medicamentos'">Ver Detalhes</button>
        ${bulaBtn}
      </div>
    `;
